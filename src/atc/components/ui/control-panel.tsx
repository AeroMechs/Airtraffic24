...
import type { MapStyle } from "@/atc/lib/map-styles";
import type { FlightState } from "@/atc/lib/opensky";
import { SearchContent } from "@/atc/components/ui/control-panel-search";
import { StyleContent } from "@/atc/components/ui/control-panel-styles";
import {
  SettingsContent,
  ShortcutsContent,
} from "@/atc/components/ui/control-panel-settings";
...
export function ControlPanel({
  ...
  useEffect(() => {
    ...
    window.addEventListener("atc:open-search", handleOpenSearch);
    window.addEventListener("atc:open-shortcuts", handleOpenShortcuts);
    return () => {
      window.removeEventListener("atc:open-search", handleOpenSearch);
      window.removeEventListener("atc:open-shortcuts", handleOpenShortcuts);
    };
  }, [open]);
    ...
    <>
      {MAIN_TABS.map(({ id, icon: Icon, label }) => (
        <motion.button
          key={id}
          onClick={() => open(id)}
          className="flex h-9 w-9 items-center justify-center rounded-lg backdrop-blur-2xl transition-colors"
          style={{
            borderWidth: 1,
...
function PanelDialog({
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col sm:flex-row overflow-hidden rounded-xl sm:rounded-2xl border border-border bg-popover shadow-[0_40px_100px_rgba(0,0,0,0.25)] dark:shadow-[0_40px_100px_rgba(0,0,0,0.8),0_0_0_1px_rgba(255,255,255,0.04)_inset] h-[75vh] sm:h-auto sm:max-h-[85vh]">
          ...
          <div className="flex flex-1 flex-col min-h-0 sm:h-120">
            ...
            <div className="hidden sm:flex items-center justify-between px-5 pt-5 pb-2">
              ...
              <motion.button
                onClick={onClose}
                className="flex h-7 w-7 items-center justify-center rounded-lg bg-foreground/6 transition-colors hover:bg-foreground/12"
          ...
          <div className="flex sm:hidden items-center gap-0.5 border-t border-border px-2 pt-2 pb-3">
            ...
            <motion.button
              onClick={onClose}
              className="ml-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-foreground/6 transition-colors active:bg-foreground/12"
              whileTap={{ scale: 0.9 }}
              aria-label="Close"
            >
