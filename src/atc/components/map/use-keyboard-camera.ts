...
export function useKeyboardCamera(
  ...
  useEffect(() => {
    ...
    window.addEventListener("atc:camera-start", onStart);
    window.addEventListener("atc:camera-stop", onStop);

    return () => {
      window.removeEventListener("atc:camera-start", onStart);
      window.removeEventListener("atc:camera-stop", onStop);
