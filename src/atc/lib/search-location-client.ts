...
//   3. Nominatim places (async, global geocoding via /api/atc/geocode proxy)
//
...
import type { NominatimResult } from "@/app/api/atc/geocode/route";
...
export async function searchGeocode(
  ...
  try {
    const res = await fetch(
      `/api/atc/geocode?q=${encodeURIComponent(normalized)}`,
      {
